...
public class DriverUserController_GET {
    ...
    public ResponseEntity<DriverUser> getDriverUser_ByEmail(@RequestParam("email") String parameter_email) {
        boolean check = driverUserService.check_available_email.test(parameter_email);
        if (!check){ DriverUser target = driverUserService.FindDriverUserByEmail(parameter_email);
